...
- **安全**：管理 API 令牌认证、同源隔离、密码用 scrypt 哈希、ZIP 解压防路径穿越 / 符号链接 / 炸弹
...
> **注意**：管理 API 使用登录时签发的内存令牌（`X-API-Token`），**刷新页面后需要重新登录**——这是发布页与管理台同源时的安全设计。
...
除公开页面（`/p/{slug}`）外，所有 `/api/*` 接口需携带登录返回的 `X-API-Token` 请求头：
...
- **同源风险**：管理台与公开页面共用同一服务。已通过「内存 API 令牌」机制隔离——仅凭会话 cookie 无法调用管理接口，同源发布页上的恶意脚本拿不到令牌。**更彻底的方案**是把公开页面反代到独立域名（如 `page.example.com`），管理台保持独立。
